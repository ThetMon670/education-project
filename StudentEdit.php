...
$StudentID=isset($_GET['StudentID']) ? $_GET['StudentID'] : $_POST['txtStudentID'];
$query="SELECT * FROM studenttb WHERE StudentID='$StudentID'"; 
$ret = mysqli_query($connection,$query);
// add data into array columns
$array = mysqli_fetch_array($ret);
$StudentID = $array['StudentID'];
$StudentPicture = $array['StudentPicture'];
...
if (isset($_POST['btnedit'])) 
{
    ...
    if (isset($_FILES['Profileimg']) && $_FILES['Profileimg']['error'] === UPLOAD_ERR_OK) 
    {
      ...
      if(!$copyimg)
      {
        ...
        $Update = "UPDATE studenttb SET StudentID = '$StudentID', StudentName = ' $StudentName', Username = '$Username', Address = '$Address', PhoneNo = '$PhoneNo', Email = '$Email', Password = '$Password', StudentPicture='$dbstorefilename' ";
        $UpdateRet = mysqli_query($connection, $Update);
        if ($UpdateRet) //True
        {
            echo "<script>window.alert('Student Information Updated')</script>";
    ...
    {
        // Perform the insertion query
        $Update = "UPDATE studenttb SET StudentID = '$StudentID', StudentName = ' $StudentName', Username = '$Username', Address = '$Address', PhoneNo = '$PhoneNo', Email = '$Email', Password = '$Password', StudentPicture='$StudentPicture' ";
        $UpdateRet = mysqli_query($connection, $Update);
        if ($UpdateRet) //True
        {
            echo "<script>window.alert('Student Information Updated')</script>";
...
<head>
    ...
    <title>MMS IT/StudentEditForm</title>
  ...
  <nav class="flex items-center justify-between flex-wrap bg-lime-700  p-6">
    ...
    <div class="flex flex-col justify-center font-[sans-serif] p-32" id="app">
            <div class="max-w-md w-full mx-auto border border-gray-300 rounded-2xl p-8">
              <form class="space-y-6" action="StudentEdit.php" method="POST" enctype="multipart/form-data">
                    <div class="sm:mx-auto sm:w-full sm:max-w-sm">
                      <img class="mx-auto h-10 w-auto" src="https://mms-it.com/profile/images/logo.png" >
                      <h2 class="mt-5 mb-5 text-center text-2xl/9 font-bold tracking-tight text-black-700">Student Edit Form</h2>
                    </div>
                    <div>
                        <label for="studentname" class="block text-sm/6 font-medium text-gray-900">Staff Name</label>
                        <div class="mt-2">
                        <input type="studentname" name="txtStudentName" id="studentname" value="<?php echo $array['StudentName'] ?>" required class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 
                    ...
                    <div>
                        ...
                        <div class="mt-2 block text-sm/6 font-medium text-gray-900">
                            <img id="oldimg" name="oldimg" src="<?php echo $StudentPicture; ?>">
                        </div>
                    </div>
                    <div>
